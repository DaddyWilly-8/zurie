...
export const normalizeExchangeRates = (
  ...
  return {
    USD:
      rates?.USD && rates.USD > 0 ? rates.USD : DEFAULT_USD_EXCHANGE_RATE.USD,
    TZS:
      rates?.TZS && rates.TZS > 0 ? rates.TZS : DEFAULT_USD_EXCHANGE_RATE.TZS,
    EUR:
      rates?.EUR && rates.EUR > 0 ? rates.EUR : DEFAULT_USD_EXCHANGE_RATE.EUR,
    GBP:
      rates?.GBP && rates.GBP > 0 ? rates.GBP : DEFAULT_USD_EXCHANGE_RATE.GBP,
    KES:
      rates?.KES && rates.KES > 0 ? rates.KES : DEFAULT_USD_EXCHANGE_RATE.KES,
  };
};
...
export const formatBaseCurrencyInCurrency = (
  ...
  return formatCurrency(
    convertFromBaseCurrency(amountInBaseCurrency, currency, rates),
    currency,
  );
};
