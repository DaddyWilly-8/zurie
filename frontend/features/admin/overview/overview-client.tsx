...
type StatItem = {
  ...
  color?: string;
  subtitle: string;
  /** Permission key required to see this card — omit if every admin can. */
  permission?: string;
};
...
export const AdminOverviewClient = () => {
  ...
  if (!canViewDashboard) {
    return (
      ...
      >
        ...
        <Typography sx={{ color: getSecondaryTextColor(), fontSize: "0.9rem" }}>
          Ask an administrator to grant the &quot;View the admin dashboard
          overview&quot; permission.
        </Typography>
      </Stack>
    );
  ...
  type QuickAction = { href: string; label: string; icon: IconProp };
  ...
  return (
    <Stack spacing={3} sx={{ pb: 2 }}>
      ...
      {/* Quick Stats Row */}
      {!isLoading && !isError && (canViewOrders || canViewCustomers) && (
        <Grid container spacing={2.5}>
          {canViewOrders && (
            <>
          ...
          >
            {quickActions.map((action) => (
              <Button
                ...
                href={action.href}
                variant="outlined"
                startIcon={<FontAwesomeIcon icon={action.icon} size="sm" />}
                sx={actionButtonSx}
              >
                {action.label}
              </Button>
