...
export default async function ContactPage() {
  const settings = await contentService.getPublicSettings();
  ...
  const whatsappPhone =
    phones.find((p) => p.label.toLowerCase().includes("whatsapp")) ?? phones[0];
        ...
        <Grid size={{ xs: 12, md: 5.8 }}>
          <Stack spacing={2.5}>
            ...
            >
              ...
              {/* WhatsApp Button */}
              <Link
                href={`https://wa.me/${(whatsappPhone?.value ?? "").replace(/\D/g, "")}`}
                target="_blank"
