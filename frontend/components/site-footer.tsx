...
export const SiteFooter = async () => {
  ...
  const siteName = brand?.siteName || "Zuriè";
  const tagline =
    brand?.tagline ||
    "The architecture of elegance. Handcrafted leather goods for the modern woman.";
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            ...
            <Stack spacing={0.9}>
              {(contact?.phones ?? []).map((phone) => (
                <Typography key={phone.value} color="rgba(242,239,233,0.72)">
                  {phone.value}
                </Typography>
              ))}
              {(contact?.emails ?? []).map((email) => (
                <Typography key={email.value} color="rgba(242,239,233,0.72)">
                  {email.value}
                </Typography>
              ))}
            </Stack>
