...
export default async function HomePage() {
  const [products, featuredRows, bestSellerRows, newArrivalRows, settings] =
    await Promise.all([
      productService.getStorefrontProducts({ page: 1, pageSize: 12 }),
  ...
  const heroTitle = homepage.heroHeading || brand.siteName || "Zuriè";
  const heroSubtitle = homepage.heroSubheading || "The Atelier Collection";
  const heroDescription =
    homepage.heroSubheading ||
    "The architecture of elegance, handcrafted for the modern woman.";
  const heroButtonText = homepage.heroCtaText || "Discover The Collection";
  const heroButtonLink = homepage.heroCtaUrl || "/shop";
  const heroImageFromCms = homepage.heroImageUrl;
  const heroActive = true;
  ...
  const bannerActive: boolean = false;
  const bannerImage: string = "";
  const bannerTitle: string = "";
  const bannerDescription: string = "";
  const bannerCtaLink: string = "/shop";
  const bannerCtaText: string = "";
