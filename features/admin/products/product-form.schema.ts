...
export const productFormSchema = z
  ...
  .refine(
    (values) =>
      values.compareAt === null ||
      values.price === null ||
      values.compareAt <= values.price,
    {
  ...
  .refine(
    (values) => {
      // If status is "published", there must be at least one image
      if (values.status === "published") {
        const hasImages =
          values.imageUrlsText.trim().length > 0 ||
  ...
  .refine(
    (values) => {
      // If status is "published", stock must be greater than 0
      if (values.status === "published") {
        return values.stockCount > 0;
      }
      return true;
    },
    {
      message: "Product must have stock to be published",
      path: ["stockCount"],
    },
  );
