...
const collectNewImageFiles = async (form: ProductFormState, prefix: string) => {
  ...
  return Promise.all(
    urls.map((dataUrl, index) => dataUrlToFile(dataUrl, prefix, index)),
  );
};
...
export const productActions = {
  async list(): Promise<AdminProduct[]> {
    const productList =
      (await productService.listAdminProducts()) as AdminProduct[];
    return productList;
  },

  async getById(id: string): Promise<AdminProduct | null> {
    return productService.getAdminProductById(
      id,
    ) as Promise<AdminProduct | null>;
  },
  ...
  async update(id: string, form: ProductFormState) {
    ...
    if (form.removedImageIds.length) {
      await Promise.all(
        form.removedImageIds.map((imageId) =>
